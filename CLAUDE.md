...
│   │   ├── index.html
│   │   ├── .storybook/         # main.ts (addons + vite overrides), preview.ts (tokens + a11y)
│   │   └── src/
...
| Frontend a11y assertions | **axe-core**, via `src/test/axe.ts` and the a11y addon |
| CI | GitHub Actions |
...
7. Every component ships a `.stories.tsx`. See below — a component with no story is not covered by anything.
